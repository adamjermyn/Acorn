...
st = star.star(x,y,0.3*mSun,2.65*rSun,0.1*lSun,1.5,thermcache,rhocache,convcache,lext=0*lSun,caution=500)
...
for i in range(201):
	if i%20==0:
		plt.subplot(211)
		plt.title('$L_{in}=0.1L_{sun}, \Delta t=1e8s$')
	...
	if i<=100:
		...
		st.stepController(1e6,100*eps/100)
	print 'Step',i
fig.legend(lines,labels,loc='right')
plt.savefig('Plots/01Lsun_long_additive.eps',dpi=200)